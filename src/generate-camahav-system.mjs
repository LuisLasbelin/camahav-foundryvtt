...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'camahav', 'Camahav', and 'CAMAHAV'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.camahav/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.camahav/g),
   ...
   * Rename files.
   *
   * Rename files that had camahav in their name, such as
   * css/camahav.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*camahav*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('camahav', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-camahav-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Camahav's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace camahav name mentions in files.
      generator.replaceFileContents();
      // Rename files that had camahav in their name.
